...
export default function FinalReport({ params }: { params: { id: string } }) {
  const id = params.id;
  ...
  useEffect(() => {
    (async () => {
      setBusy(true);
      try {
        // Load report (includes details; mirrors Draft loader)
        ...
        if (e2) throw e2;
        const typedItems = (its ?? []) as ItemRow[];
        setItems(typedItems);

        // Load all photos for these items, then create signed URLs
  ...
  return (
    <main className="min-h-screen bg-gray-50 p-6">
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        {/* -------- Header card (read-only; same layout as Draft) -------- */}
